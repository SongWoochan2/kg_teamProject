package seat.bean;

public class SeatDTO {
	private int seat_num;
	private int theater_place_code;
	private int seat_type_code;
	private int x_index;
	private int y_index;
	
	public int getSeat_num() {
		return seat_num;
	}
	public void setSeat_num(int seat_num) {
		this.seat_num = seat_num;
	}
	public int getTheater_place_code() {
		return theater_place_code;
	}
	public void setTheater_place_code(int theater_place_code) {
		this.theater_place_code = theater_place_code;
	}
	public int getSeat_type_code() {
		return seat_type_code;
	}
	public void setSeat_type_code(int seat_type_code) {
		this.seat_type_code = seat_type_code;
	}
	public int getX_index() {
		return x_index;
	}
	public void setX_index(int x_index) {
		this.x_index = x_index;
	}
	public int getY_index() {
		return y_index;
	}
	public void setY_index(int y_index) {
		this.y_index = y_index;
	}
	
	
}

