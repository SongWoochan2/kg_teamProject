...
/* 		background: linear-gradient(135deg, #b3dced 0%,#18458e 40%,#bce0ee 100%);  */
	...
	div#container #content #admintitle{
		font-size: 25px;
		font-weight: bold;
		color : white;
	}
	div#container #content #adminlogin{
		box-sizing : border-box;
		margin : auto;
 		line-height : 50px; 
		width: 500px;
	...
	div#container #content #adminlogin #logintop{
		z-index : 0;
		height : 15%;
		font-size : 20px;
		padding-left : 20px;
	...
	div#container #content #adminlogin #dvadminid{
		padding-top : 30px;
		height : 25%;
		width: 100%;
	}
	div#container #content #adminlogin #dvadminid input[type='text']{
		width : 200px;
		height: 40px;
		border : 5px solid skyblue;
		border-radius: 10px;
	}
	div#container #content #adminlogin #dvadminpwd{
		height : 15%;
		width: 100%;
	}
	div#container #content #adminlogin #dvadminpwd input[type='password']{
		width : 200px;
		height: 40px;
		border : 5px solid skyblue;
		border-radius: 10px;
	}
	div#container #content #adminlogin #loginbtn{
		height : 30%;
		width: 100%;
	}
	div#container #content #adminlogin #dvloginbtn input[type='submit']{
		color : white;
		font-size : 16px;
		font-weight : bold;
		margin-top : 20px;
	...
	div#container #content #adminlogin #dvloginbtn input[type='submit']:hover{
		background-color: #0e2c5b;
		text-decoration: underline;
	}
	div#container #content #adminlogin #loginbottom{
	...
	div#container #content #adminlogin #loginbottom a{
		color: #0f2d5b;
		text-decoration: none;
	}
	div#container #content #adminlogin #loginbottom a:hover{
		text-decoration: underline;
		color: #071428;
	}
</style>
...
<body>
	<div id = "container">
		...
		<c:if test="${requestScope.target == 'adminLoginForm' }">
			<div id = "adminlogin" >
				<div id = "logintop">관리자 로그인</div>
					...
					<div id = "loginbottom">
						<a href = "#">관리자등록</a> <a href = "#">아이디/비밀번호 찾기</a>
					</div>
			</div>
		</c:if>
			<div id = "c">
		<c:if test="${requestScope.target == 'movieAdmin' }">
			<jsp:include page="../admin/movieAdmin.jsp"/>
		</c:if>
		<c:if test="${requestScope.target == 'movieInsert' }">
			<jsp:include page="../admin/movieInsert.jsp"/>
		</c:if>
		<c:if test="${requestScope.target == 'movieModify' }">
			<jsp:include page="../admin/movieModify.jsp"/>
		</c:if>
		<c:if test="${requestScope.target == 'movieView' }">
			<jsp:include page="../admin/movieView.jsp"/>
		</c:if>
			</div>
		</div>
	</div>
